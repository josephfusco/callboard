...
( () => {
	...
	// ---- Dot matrix. An entertainment display in the idiom of the late-nineties head unit, drawn by us: a
	// 5x7 glyph set, a spectrum from the live audio while a track plays, and a creature that swims across
	// while nothing does. One canvas redrawn each frame while the mode is on, on a faint static grid.
	const matrixWin = $( 'matrix-window' ),
		matrixGrid = $( 'matrix-grid' ),
		matrixText = $( 'matrix' ),
		PITCH = 3, // one dot every 3 css px: a 2px dot and a 1px gap
		ROWS = 8,
		// 95 printable ASCII glyphs, five column bytes each, bit 0 the top row
		FONT =
			'',
		glyph = ( ch ) => {
			...
			const at = ( code - 32 ) * 10;
			return [ 0, 2, 4, 6, 8 ].map( ( k ) =>
				parseInt( FONT.substr( at + k, 2 ), 16 )
			);
		},
		// text -> columns of 7-bit ints, one blank column between glyphs; accents fold to their base letter
		columns = ( text ) => {
	...
	// an original swimmer, three frames of tail, twelve columns by seven rows; row strings read top down
	const SWIM = [
		[
			'............',
			'.....##.....',
	...
	let mxData = null;
	function matrixGridPaint( W, H, cols, top, accent, dpr ) {
	...
	function matrixTitle( text ) {
		...
		mx.lastTitle = text;
		mx.text = columns( text );
		const cols = Math.floor( matrixWin.clientWidth / PITCH );
		mx.mode = 'title';
		mx.offset = mx.text.length > cols ? -cols : 0; // a long title scrolls in from the right
		mx.hold = 0;
		matrixStart();
	}
	function matrixStart() {
		if (
			! matrixWin ||
			! deck.classList.contains( 'matrix' ) ||
			deck.hidden ||
			document.hidden
		) {
			matrixStop();
		...
		if ( ! mx.raf ) {
			mx.last = 0;
			mx.raf = requestAnimationFrame( matrixFrame );
		}
	}
	function matrixStop() {
	...
	// which spectrum: the analyser's bins folded into log bands, or bars shaped from the import's envelope
	function spectrum( n, dt ) {
		if ( ! mx.bars.length ) {
			mx.bars = new Array( n ).fill( 0 );
			mx.peaks = new Array( n ).fill( 0 );
		}
		let target;
		if ( analyser ) {
			if ( ! mxData || mxData.length !== analyser.frequencyBinCount ) {
				mxData = new Uint8Array( analyser.frequencyBinCount );
			}
			analyser.getByteFrequencyData( mxData );
			target = mx.bars.map( ( _, k ) => {
				const lo = Math.floor( Math.pow( mxData.length / 2, k / n ) ),
					hi = Math.max(
						lo + 1,
						Math.floor(
							Math.pow( mxData.length / 2, ( k + 1 ) / n )
						)
					);
				let sum = 0;
				for ( let b = lo; b < hi; b++ ) {
				...
				return Math.min( 1, ( sum / ( hi - lo ) / 255 ) * 0.95 );
			} );
		} else {
			const e = envelope( audio.currentTime ),
				level = e === null ? 0.3 : e,
				t = audio.currentTime;
			target = mx.bars.map( ( _, k ) =>
				Math.max(
					...
					Math.min(
						1,
						level *
							( 0.55 + 0.45 * Math.sin( k * 1.7 + t * 7 + k ) ) *
							( 1.15 - k / n / 2 )
					)
				)
			);
		...
		mx.bars = mx.bars.map(
			( v, k ) =>
				v +
				( target[ k ] - v ) *
					( target[ k ] > v ? 0.6 : Math.min( 1, dt * 6 ) )
		);
		mx.peaks = mx.peaks.map( ( pk, k ) =>
			mx.bars[ k ] >= pk ? mx.bars[ k ] : Math.max( 0, pk - dt * 0.9 )
		);
	}
	function matrixFrame( now ) {
		...
		if ( matrixGrid.width !== Math.round( W * dpr ) ) {
			matrixGridPaint( W, H, cols, top, accent, dpr );
		}
		if ( matrixText.width !== Math.round( W * dpr ) ) {
			matrixText.width = Math.round( W * dpr );
			matrixText.height = Math.round( H * dpr );
			matrixText.style.width = `${ W }px`;
		}
		...
		const dot = ( k, r ) => {
			if ( k >= 0 && k < cols ) {
				m.fillRect( k * PITCH, top + r * PITCH, 2, 2 );
			}
		};
		const playing = ! audio.paused,
			still = reduce();
		// the title: static if it fits, otherwise it scrolls through once, then hands over
		if ( mx.mode === 'title' ) {
			const long = mx.text.length > cols;
			if ( long && ! still ) {
				mx.offset += dt * 22; // columns per second
				if ( mx.offset > mx.text.length + 4 ) {
					mx.mode = playing ? 'spectrum' : 'swim';
					mx.hold = 0;
					mx.t0 = now;
				}
			} else {
				mx.hold += dt;
				if ( mx.hold > ( playing ? 4 : 8 ) && ! still ) {
					mx.mode = playing ? 'spectrum' : 'swim';
					mx.t0 = now;
				}
			}
			const start = long ? -Math.floor( mx.offset ) : 0;
			mx.text.forEach( ( bits, k ) => {
				for ( let r = 0; r < 7; r++ ) {
					if ( bits & ( 1 << r ) ) {
						dot( start + k, r );
					}
		...
		} else if ( mx.mode === 'spectrum' ) {
			if ( ! playing ) {
				mx.mode = 'swim';
				mx.t0 = now;
			}
			const n = Math.min( 24, Math.floor( cols / 4 ) );
			spectrum( n, dt );
			const left = Math.floor( ( cols - n * 4 ) / 2 );
			mx.bars.forEach( ( v, k ) => {
				const h = Math.round( v * ROWS );
				for ( let r = 0; r < h; r++ ) {
					dot( left + k * 4, ROWS - 1 - r );
					dot( left + k * 4 + 1, ROWS - 1 - r );
					dot( left + k * 4 + 2, ROWS - 1 - r );
				}
				const pk = Math.round( mx.peaks[ k ] * ROWS );
				if ( pk > h ) {
					dot( left + k * 4, ROWS - pk );
					dot( left + k * 4 + 1, ROWS - pk );
					dot( left + k * 4 + 2, ROWS - pk );
				}
			} );
			if ( now - mx.t0 > 25000 ) {
				matrixTitle( mx.lastTitle ); // the title comes round again
				return;
			}
		} else if ( mx.mode === 'swim' ) {
			if ( playing ) {
				mx.mode = 'spectrum';
				mx.t0 = now;
			}
			// the swimmer crosses left to right over eight seconds, bobbing, with a bubble trailing
			const tt = ( ( now - mx.t0 ) / 1000 ) % 11,
				x = Math.round( ( tt / 8 ) * ( cols + 14 ) ) - 13,
				bob = Math.round( Math.sin( tt * 2.2 ) * 0.9 ),
				frame = SWIM[ Math.floor( ( now / 220 ) % 3 ) ];
			if ( tt < 8 ) {
				frame.forEach( ( bits, c ) => {
					for ( let r = 0; r < 7; r++ ) {
						if ( bits & ( 1 << r ) ) {
							dot( x + c, r + bob );
						}
				...
				if ( Math.floor( tt * 2 ) % 2 ) {
					dot( x - 2, 1 - ( Math.floor( tt ) % 2 ) );
				}
			}
		}
		if ( still && mx.mode !== 'title' ) {
			matrixTitle( mx.lastTitle );
			return;
		}
		mx.raf = requestAnimationFrame( matrixFrame );
	}
	const paintMatrix = ( text ) => matrixTitle( text );
	audio.addEventListener( 'play', () => {
		if ( mx.mode === 'swim' ) {
			mx.mode = 'spectrum';
			mx.t0 = performance.now();
		}
		matrixStart();
	} );
	audio.addEventListener( 'pause', matrixStart );
	document.addEventListener( 'visibilitychange', () =>
		document.hidden ? matrixStop() : matrixStart()
	);
